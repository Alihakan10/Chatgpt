...
# NATIVE_30M_ONLY
# Tum taramalar TradingView native 30 dakikalik seri ile yapilir.
# 30M -> 30M manuel birlestirme KULLANILMAZ.
MANUAL_TEST_RUN = (
    os.getenv(
        "MANUAL_TEST_RUN",
        "false"
...
STATE_FILE = (
    "state/supertrend_state_30m_tvparity.json"
)
...
def get_tv_candles(symbol, candle_mode="native_30m", candle_session="regular"):

    if candle_mode not in ("native_30m", "native_30m"):
    ...
    try:
        ...
        # TradingView 30M bars are session-anchored. For BIST's 10:00 session
        # a 30M chart should start at 10:00, not at the UTC/calendar epoch.
        # The native TradingView series already supplies the exchange-session bars;
        # do not manually aggregate 30M data.
        ws.send(
            tv_message(
                "create_series",
                [
        ...
        if len(result) < 20:
            raise RuntimeError(
                "TradingView native 30M verisi yetersiz: "
                + str(len(result)) + " mum"
            )
...
def get_last_completed_index(
    ...
    """
    Native TradingView 30M serisinde BIST regular seansinin son parcali
    30M barini da tamamlanmis kabul eder.
...
def get_tv_candles_with_retry(symbol, candle_mode="native_30m", candle_session="regular"):
    """
...
def aggregate_completed_60m_from_30m(candles):
    """Build session-aligned 60M bars from completed native 30M TradingView bars."""
    groups = {}
    for candle in candles:
        ...
        key = int(bucket.astimezone(ZoneInfo("UTC")).timestamp())
...
def scan_symbol(
    ...
    try:
        ...
        # TradingView'in kendi native 30 dakikalik serisini kullan.
        # 30M mumlari manuel olarak birlestirmiyoruz; boylece TradingView
        # grafigindeki 30M bar sinirlarini bozmayiz.
        candle_mode = "native_30m"
        ...
        # Tum gun icindeki TAMAMLANMIS 30M mumlari hesaba kat.
        ...
        calculation_candles = aggregate_completed_60m_from_30m(candles[:completed_index + 1])

        if len(calculation_candles) < ATR_PERIOD + 2:
            return {
                "status": "skip",
                "symbol": symbol,
                "error": "60M Supertrend icin yeterli mum yok."
            }

        completed_index = len(calculation_candles) - 1
