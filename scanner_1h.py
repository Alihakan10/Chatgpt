# ============================================================
# BIST 1H SUPERTREND ALARM SISTEMI
# SURUM 2 - FINAL
# ============================================================
#
# OZELLIKLER
...
# 2) TradingView WebSocket ile native 1 saatlik (60 dakika) mum verisini alir
# 3) Supertrend:
#       ATR Period     = 10
#       Source         = HL2
#       Multiplier     = 2
...
# NATIVE_1H_ONLY
# Tum taramalar TradingView native 60 dakikalik seri ile yapilir.
# 1H -> 1H manuel birlestirme KULLANILMAZ.
MANUAL_TEST_RUN = (
    os.getenv(
        "MANUAL_TEST_RUN",
        "false"
...
STATE_FILE = (
    "state/supertrend_state_1h.json"
)
...
def get_tv_candles(symbol, candle_mode="native_1h", candle_session="regular"):

    if candle_mode not in ("native_1h", "native_1h"):
    ...
    try:
        ...
        # TradingView 1H bars are session-anchored. For BIST's 10:00 session
        # a 1H chart should start at 10:00, not at the UTC/calendar epoch.
        ...
        ws.send(
            tv_message(
                ...
                [
                    ...
                    "s1",
                    "sds_sym_1",
                    ("120" if candle_mode == "native_1h" else "60"),
                    CANDLE_COUNT,
                    ""
                ]
            )
        ...
        if len(result) < 20:
            raise RuntimeError(
                "TradingView native 1H verisi yetersiz: "
                + str(len(result)) + " mum"
            )
...
def calculate_kivanc_supertrend_directions(
    ...
    Bu fonksiyon Study calistirmaz; TradingView native 1H OHLC
...
def get_last_completed_index(
    ...
    """
    Native TradingView 1H serisinde BIST regular seansinin son parcali
    1H barini da tamamlanmis kabul eder.
    ...
    for i, candle in enumerate(candles):
        try:
            ...
            # bitiminde kapanir. 17:00 -> 18:00 parcali 1H bardir.
            if (
                candle_time.hour == 17
...
def build_telegram_message(
    ...
    lines.append(
        "Sinyal: Yalnızca SON TAMAMLANMIŞ 1H mumunda "
        "SAT -> AL (BUY) dönüşü."
    )
...
def candle_close_datetime(candle_time):
    ...
    # BIST'in 17:00-18:00 son parcali 1H bari 18:00'de kapanir.
...
def get_tv_candles_with_retry(symbol, candle_mode="native_1h", candle_session="regular"):
    """
...
def scan_symbol(
    ...
    try:
        ...
        # TradingView'in kendi native 60 dakikalik serisini kullan.
        # 1H mumlari manuel olarak birlestirmiyoruz; boylece TradingView
        # grafigindeki 1H bar sinirlarini bozmayiz.
        candle_mode = "native_1h"
        ...
        # Tum gun icindeki TAMAMLANMIS 1H mumlari hesaba kat.
        ...
        # KRITIK: SADECE SON TAMAMLANMIS 1H MUMU BUY ISE SINYAL KABUL ET.
...
def build_current_report_message(results):
    ...
    lines.append("")
    lines.append("Bu rapor yalnızca son tamamlanmış 1H mumunda BUY etiketi oluşanları gösterir.")
    lines.append("Ayarlar: ATR 10 | HL2 | 2.0 | RMA | 1H")
...
def main():
    ...
    log("=" * 70)
    log(
        "BIST 1H SUPERTREND FINAL TARAMASI BASLADI"
    )
    log("=" * 70)
    ...
    for number, symbol, result in scan_results:
        ...
        if buy_results:
            ...
            for buy_result in buy_results:
                log(
                    f"[{number}/{total}] {symbol} >>>>>> 1H YENI BUY <<<<<< "
    ...
    if FORCE_SCAN:
        ...
        log(
            f"1H YENI BUY: {len(new_buy_results)}"
        )
    ...
    # --------------------------------------------------------
    # SEND_SCAN_REPORT=true iken yalnızca son tamamlanmış 1H mumunda
